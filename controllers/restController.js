...
const deletePost = async (req,res) => {
    ...
    try {
        ...
        const userPost = await restModel.gettingPost(postId);
        if (deletePost == true) {
            res.render('./pages/post-view',{title: 'Your Post',message:'You successfully deleted your post from your profile and from our servers',post:userPost[0],userPost});
        } else {
            res.render('./pages/post-view',{title: 'Your Post',message:'Something went wrong, please try again'});
        }
    } catch (error) {
        console.error('Error:', error);
        res.status(500).send('Internal Server Error');
